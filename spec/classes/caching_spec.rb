    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context 'with non_chroot' do
          ...
          it { is_expected.not_to create_file('/etc/named.conf').with(target: '/var/named/chroot/etc/named.conf') }
        ...
        context 'when trying to include ::named' do
          ...
          it { expect { is_expected.to compile.with_all_deps }.to raise_error(%r{cannot include both}) }
        end
      end
    end
  end
